...
export default function Home() {
  ...
  useEffect(() => {
    setIsLoading(true);
    Promise.all([
      fetch(`/api/projects?locale=${locale}`).then(res => res.json()).catch(() => []),
      fetch(`/api/certificates?locale=${locale}`).then(res => res.json()).catch(() => [])
    ])
        ...
        >
          ...
          <motion.div variants={itemVariants} className="flex flex-wrap gap-4 items-center">
            ...
            >
              ...
              <span className="relative">{tm('projects')}</span>
            </Link>
            <Link
              href="/about"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {isLoading ? (
                ...
                <div className="col-span-1 sm:col-span-2 py-12 text-center border border-dashed border-slate-700/50 rounded-2xl bg-slate-800/20">
                  <p className="text-slate-500 text-sm">{t('noProjects')}</p>
                </div>
              )}
            </div>
